...
# Madde 5: seçilen yıl/dönem itibarıyla portföyde GERÇEKTEN var olan daire sayısı
if selected_years:
...
st.caption(
    "Her sözleşmenin toplam tutarı, sözleşme süresine bölünerek güne yayılır (örn. 700₺/7 gün = günlük 100₺ -- "
    "aralığın sadece bir kısmı seçilse bile günlük pay değişmez, sadece görünen günler kırpılır). "
    "Bugünden sonraki (henüz yaşanmamış ama sözleşme kapsamındaki) günler kesikli çizgiyle gösterilir."
)
...
if dr_start > dr_end:
    ...
    if not daily_rows:
        ...
        # Süreklilik için: geçmiş çizginin son noktasını gelecek çizginin başına da ekle,
        # böylece dolu çizgi ile kesikli çizgi görsel olarak kopuk durmaz.
        ...
        if not past_df.empty:
            fig_daily.add_trace(go.Scatter(
                ...
                mode="lines", line=dict(color="#319795", width=2), fill="tozeroy", fillcolor="rgba(49,151,149,0.15)",
        ...
        if not future_df.empty:
            fig_daily.add_trace(go.Scatter(
                ...
                mode="lines", line=dict(color="#319795", width=2, dash="dash"),
...
st.caption("Sıralama ve dilim büyüklükleri her zaman TL karşılığı üzerinden hesaplanır (döviz cinsinden ödemeler dahil).")
...
with col_pay1:
    ...
    if not df_method.empty:
        ...
        fig_method.update_layout(height=360, margin=dict(l=10, r=10, t=50, b=20))
...
with col_pay2:
    ...
    if not df_currency.empty:
        ...
        fig_currency.update_layout(height=360, margin=dict(l=10, r=10, t=50, b=20))
