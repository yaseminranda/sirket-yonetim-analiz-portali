...
# Not: Bu sayfa hem araç hem ev departmanında ortak kullanıldığı için tüm
# çalışanlara açık; departman/rol kısıtı yok (proje planlama konuşmasındaki
# karar). Yeni müşteri ekleme burada YOK -- müşteri sadece sözleşme
# oluştururken eklenir.

st.title("👥 Müşteriler")
st.caption("Müşterileri arayın, bilgilerini görüntüleyin, düzenleyin ve sözleşme geçmişlerini inceleyin.")
st.divider()

# Sözleşmeler / kiralama sayfalarından yönlendirme ile buraya gelinmişse
# önceden seçili müşteriyi otomatik olarak öne getir.
...
selected = st.selectbox("Müşteri Seçiniz:", options, index=default_index, key="musteri_secim")
        ...
        with st.form("musteri_duzenle_form"):
            ...
            if submitted:
                result = api_put(
                    ...
                    {"isim": isim, "telefon": telefon, "email": email, "tc_kimlik_no": tc_no},
                )
