"""Streamlit page for transferring an active vehicle rental contract to a replacement vehicle (e.g. after a breakdown)."""
...
def get_contracts_df() -> pd.DataFrame:
    """Fetch all vehicle contracts from the API and return them as a DataFrame with normalized date columns."""
...
if selected != "--- Seçiniz ---":
    ...
    remaining_debt = float(pd.to_numeric(row.get("kalan_borc"), errors="coerce") or 0)
    if remaining_debt > 0:
        st.warning(
            f"⚠️ Bu sözleşmede **₺{remaining_debt:,.2f}** ödenmemiş bakiye var. Araç değişimi bu "
            "bakiyeyi silmez ya da yeni sözleşmeye taşımaz -- borç, eski (kısaltılmış) sözleşme "
            f"#{contract_no} üzerinde kalmaya devam edecek ve 'Araba Kiralama Ödeme Yönetimi' "
            "sayfasından tahsil edilebilecek."
        )
