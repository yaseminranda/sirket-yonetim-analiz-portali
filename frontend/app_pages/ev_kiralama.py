"""Streamlit page for managing housing rental contracts: overdue/expiry tracking, creating new rentals, vacate/cancellation, and the full contract list."""
...
def get_contracts_df() -> pd.DataFrame:
    """Fetch all housing rental contracts (cached) and return them as a DataFrame with parsed start/end dates."""
    ...
    if df_house.empty:
        ...
        with t_overdue:
            ...
            if not df_o.empty:
                ...
                def highlight_red_rows(row):
                    """Style a DataFrame row with a red highlight to flag overdue contracts."""
                ...
                st.markdown("---")
                st.markdown("#### 🚨 Geciken Kiracıya İhtar Gönder")
                sel = st.selectbox("Sözleşme Seçin:", df_o["sozlesme_no"].tolist(), key="sb_ev_geciken_ihtar")
                ihtar_col1, ihtar_col2 = st.columns(2)
                with ihtar_col1:
                    if st.button("📧 E-posta ile Gönder", key="btn_ev_geciken_email", use_container_width=True):
                        result = api_post(f"/housing/contracts/{sel}/notify-overdue?method=email")
                ...
                with ihtar_col2:
                    if st.button("📲 SMS ile Gönder", key="btn_ev_geciken_sms", use_container_width=True):
                        result = api_post(f"/housing/contracts/{sel}/notify-overdue?method=sms")
                        if result and result.get("success"):
                            st.session_state["toast_mesaj"] = result["message"]
                            st.session_state["toast_icon"] = "📲"
                            st.rerun()
        ...
        with t_rent_overdue:
            ...
            if rent_data:
                ...
                st.markdown("---")
                st.markdown("#### 🚨 Geciken Kira Borcu İçin İhtar Gönder")
                sel2 = st.selectbox("Sözleşme Seçin:", df_rent["sozlesme_no"].tolist(), key="sb_ev_kira_geciken")
                kira_ihtar_col1, kira_ihtar_col2 = st.columns(2)
                with kira_ihtar_col1:
                    if st.button("📧 E-posta ile Gönder", key="btn_ev_kira_email", use_container_width=True):
                        result = api_post(f"/housing/contracts/{sel2}/notify-overdue-rent?method=email")
                        if result and result.get("success"):
                            st.session_state["toast_mesaj"] = result["message"]
                            st.session_state["toast_icon"] = "📧"
                            st.rerun()
                            ...
                            st.info(result["message"])
                with kira_ihtar_col2:
                    if st.button("📲 SMS ile Gönder", key="btn_ev_kira_sms", use_container_width=True):
                        result = api_post(f"/housing/contracts/{sel2}/notify-overdue-rent?method=sms")
                        if result and result.get("success"):
                            st.session_state["toast_mesaj"] = result["message"]
                            st.session_state["toast_icon"] = "📲"
                            st.rerun()
    ...
    if end_date < start_date:
        ...
        if not apartments:
            ...
            if is_apt_selected:
                ...
                deposit_amount = st.number_input("Depozito Tutarı (₺):", min_value=0.0, value=float(monthly_rent), step=500.0)
                st.info(
            ...
            if st.button("✅ Ev Kiralamasını Onayla ve Başlat", type="primary"):
                if selected_customer == "--- Seçiniz ---":
                    ...
                    result = api_post("/housing/contracts", payload, timeout=30)
                    if result and result.get("success"):
...
with tab4:
    ...
    if df_list.empty:
        ...
        def highlight_overdue(row):
            """Style a DataFrame row with a red highlight when its contract is active but past its end date."""
