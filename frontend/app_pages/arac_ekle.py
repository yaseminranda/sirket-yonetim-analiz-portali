"""Streamlit page for managing the vehicle fleet: adding new vehicles, retiring existing ones, and listing the fleet."""
...
with tab_add:
    ...
    if st.button("✅ Aracı Filoya Ekle", type="primary"):
        if not plaka:
            st.error("⚠️ Lütfen plaka giriniz.")
        elif is_new_brand and not new_brand_name.strip():
            st.error("⚠️ Yeni marka eklemek için marka adını giriniz.")
        elif is_new_model and not new_model_name.strip():
            st.error("⚠️ Yeni model eklemek için model adını giriniz.")
        elif gunluk_ucret is None or gunluk_ucret <= 0:
            st.error("⚠️ Lütfen geçerli (0'dan büyük) bir günlük kiralama ücreti giriniz.")
        else:
            payload = {
