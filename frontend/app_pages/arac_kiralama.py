"""Streamlit page for managing vehicle rental contracts: overdue/expiry tracking, creating new rentals, returns/cancellations, and the full contract list."""
...
def render_scrollable_table(df: pd.DataFrame, highlight_col: str, highlight_value, height_px: int = 500) -> None:
    """Render a DataFrame as a scrollable HTML table, highlighting rows where highlight_col equals highlight_value."""
...
def get_contracts_df() -> pd.DataFrame:
    """Fetch all vehicle rental contracts (cached) and return them as a DataFrame with parsed start/end dates."""
        ...
        with t_overdue:
            ...
            if not df_overdue.empty:
                ...
                def highlight_red_rows(row):
                    """Return a red highlight style for every cell in the given row."""
                ...
                st.markdown("---")
                st.markdown("#### 🚨 Geciken Müşteriye İhtar Gönder")
                sel = st.selectbox("Sözleşme Seçin:", df_overdue["sozlesme_no"].tolist(), key="sb_arac_geciken_ihtar")
                ihtar_col1, ihtar_col2 = st.columns(2)
                with ihtar_col1:
                    if st.button("📧 E-posta ile Gönder", key="btn_arac_geciken_email", use_container_width=True):
                        result = api_post(f"/vehicles/contracts/{sel}/notify-overdue?method=email")
                ...
                with ihtar_col2:
                    if st.button("📲 SMS ile Gönder", key="btn_arac_geciken_sms", use_container_width=True):
                        result = api_post(f"/vehicles/contracts/{sel}/notify-overdue?method=sms")
                        if result and result.get("success"):
                            st.session_state["toast_mesaj"] = result["message"]
                            st.session_state["toast_icon"] = "📲"
                            st.rerun()
    ...
    if end_date < start_date:
        ...
        if not vehicles:
            ...
            def _driver_error(name, phone, email, tc, label):
                """Validate an additional driver's fields, returning an error message if partially filled or invalid, else None."""
