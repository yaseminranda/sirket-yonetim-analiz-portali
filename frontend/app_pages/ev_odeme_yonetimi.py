"""Streamlit page for managing housing rental payments, installment plans, and payment history."""
...
if selected != "--- Seçiniz ---":
                ...
                if st.button("✅ Taksit Ödemesini Kaydet", type="primary", key="ev_taksit_odeme_btn"):
                    ...
                    if result and result.get("success"):
                        invoice_bytes = api_get_raw(
                            ...
                            {
                                "customer_name": c_row["musteri_adi"], "amount": t_amount,
                                "transaction_type": "Ev Kiralama Taksit Tahsilatı", "remaining_balance": remaining_debt - t_amount,
                            },
                        )
                        st.session_state["ev_taksit_odeme_basarili"] = {
                            "contract_no": contract_no, "message": result["message"], "invoice_bytes": invoice_bytes,
                        }
                    elif result:
                        st.error(result["message"])

                taksit_pending = st.session_state.get("ev_taksit_odeme_basarili")
                if taksit_pending and taksit_pending["contract_no"] == contract_no:
                    st.success(taksit_pending["message"])

                    @st.fragment
                    def _ev_taksit_fatura_indirme_fragmani():
                        if taksit_pending.get("invoice_bytes"):
                            st.download_button(
                                "📄 Makbuz / Fatura İndir (Excel)", data=taksit_pending["invoice_bytes"],
                                file_name=f"Fatura_Ev_{contract_no}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                key="download_fatura_ev_taksit",
                            )

                    _ev_taksit_fatura_indirme_fragmani()
                    if st.button("📋 Sözleşme Detaylarına / Ödeme Geçmişine Git", key="ev_taksit_detaya_git_btn"):
                        del st.session_state["ev_taksit_odeme_basarili"]
    ...
    if remaining_debt <= 0:
        ...
        if st.button("✅ Ödemeyi Kaydet ve Tahsil Et", type="primary"):
            ...
            if result and result.get("success"):
                invoice_bytes = api_get_raw(
                    f"/finance/invoice/{contract_no}",
                    {
                        "customer_name": c_row["musteri_adi"], "amount": payment_amount,
                        "transaction_type": "Ev Kiralama Tahsilatı", "remaining_balance": remaining_debt - payment_amount,
                    },
                )
                st.session_state["ev_odeme_basarili"] = {
                    "contract_no": contract_no, "tutar": payment_amount, "invoice_bytes": invoice_bytes,
                }
            elif result:
                st.error(result["message"])
        ...
        if pending and pending["contract_no"] == contract_no:
            ...
            @st.fragment
            def _ev_fatura_indirme_fragmani():
                if pending.get("invoice_bytes"):
                    st.download_button(
                        "📄 Makbuz / Fatura İndir (Excel)", data=pending["invoice_bytes"], file_name=f"Fatura_Ev_{contract_no}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="download_fatura_ev",
                    )

            _ev_fatura_indirme_fragmani()
            if st.button("📋 Sözleşme Detaylarına / Ödeme Geçmişine Git", key="ev_detaya_git_btn"):
