"""Streamlit page for managing the housing portfolio: adding new units, retiring existing ones, and listing the portfolio."""
...
with tab_add:
    ...
    if st.button("✅ Daireyi Portföye Ekle", type="primary"):
        if not daire_no.strip():
            st.error("⚠️ Lütfen daire no giriniz.")
        elif not oda_sayisi.strip():
            st.error("⚠️ Lütfen oda sayısı giriniz.")
        elif is_new_building and not (new_apartman_adi.strip() and new_il.strip() and new_ilce.strip()):
            st.error("⚠️ Yeni apartman/bina eklemek için Apartman Adı, İl ve İlçe alanlarının tümünü doldurunuz.")
        elif aylik_kira is None or aylik_kira <= 0:
            st.error("⚠️ Lütfen geçerli (0'dan büyük) bir aylık kira bedeli giriniz.")
        else:
            payload = {
